...
export class MusicCommand extends Command {
    async messageRun(msg: Message, args: Args) {
        ...
        let now = new MessageEmbed()
            ...
            .setFooter(`${toColonNotation(player?.position)} ${progress(bar, current, total, slider, size)[0]} ${player?.queue?.current?.isStream ? 'LIVE' : toColonNotation(player?.queue?.current?.duration)}`)
